...
The mean model will use an ARMA(1,1) and we will use different GARCH orders to find the best log-likelihood value to evaluate goodness of fit.
...
APARCH_spec <- ugarchspec(variance.model = list(model = "apARCH", garchOrder = c(2,2)),
                   mean.model = list(armaOrder = c(1,1), include.mean = TRUE)) 
...
IGARCH_spec <- ugarchspec(variance.model = list(model = "iGARCH", garchOrder = c(0,1)),
                   mean.model = list(armaOrder = c(1,1), include.mean = TRUE)) # Integrated GARCH
