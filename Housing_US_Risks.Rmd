...
arma11_model<- sarima(mspus.box.diff, p = 1, d = 0, q = 1)
```
...
GARCH models assume stationarity. Therefore we will use the differenced series for GARCH model which is stationary.
...
GARCH_spec <- ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
                   mean.model = list(armaOrder = c(1,1), include.mean = TRUE)) # Standard GARCH
...
APARCH_spec <- ugarchspec(variance.model = list(model = "apARCH", garchOrder = c(1, 1)),
                   mean.model = list(armaOrder = c(1,1), include.mean = TRUE)) # Asymmetric Power ARCH
GARCH_fit <- ugarchfit(spec = GARCH_spec, data = mspus.box.diff)
IGARCH_fit <- ugarchfit(spec = IGARCH_spec, data = mspus.box.diff)
APARCH_fit <- ugarchfit(spec = APARCH_spec, data = mspus.box.diff)
