...
```
GARCH models assume stationarity. Therefore we will use the log-differenced series for GARCH model which was shown to be stationary earlier.

```{r}
acf(mspus.box.diff)
pacf(mspus.box.diff)

sarima101 <- sarima(mspus.ts.boxcox, p = 1, d = 1, q = 1)
summary(sarima101)
sarima.for(mspus.ts.boxcox, p = 1, d = 1, q = 1, n.ahead = 5)
```
